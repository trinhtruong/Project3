...
uint8_t sensor_readReg(uint8_t regID) {
   uint8_t regDat;
    Wire.beginTransmission(OV7670_RD_ADDR >> 1);
    Wire.write(regID & 0x00FF);    
    Wire.endTransmission();
    Wire.requestFrom((OV7670_RD_ADDR >> 1),1);
    if(Wire.available()) {
        regDat = Wire.read();
         //delay(1);
        return regDat;
